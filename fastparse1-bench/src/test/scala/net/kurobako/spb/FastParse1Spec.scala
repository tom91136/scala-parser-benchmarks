...
import net.kurobako.spb._
import FastParse1Bench.{BrainFuckBench, JsonBench, SimpleBench}
...
class FastParse1Spec extends FlatSpec with Matchers with EitherValues {
	...
	"simple" should "parse according to spec" in {
		...
		forAll(Table(
			("def", "input", "expected"),
			(simple.warmFastParse1BindFoldCurried _, TestString, Expected),
			(simple.warmFastParse1BindFoldTuple2 _, TestString, Expected),
			(simple.warmFastParse1BindRecursiveCurried _, TestString, Expected),
			(simple.warmFastParse1BindRecursiveTuple2 _, TestString, Expected),
			(simple.warmFastParse1RightRecursive _, TestString, Expected),
			(simple.coldFastParse1BindFoldCurried _, TestString, Expected),
			(simple.coldFastParse1BindFoldTuple2 _, TestString, Expected),
			(simple.coldFastParse1BindRecursiveCurried _, TestString, Expected),
			(simple.coldFastParse1BindRecursiveTuple2 _, TestString, Expected),
			(simple.coldFastParse1RightRecursive _, TestString, Expected)
